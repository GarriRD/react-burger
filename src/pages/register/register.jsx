...
const Register = () => {
  ...
  return (
    <form className='form-page' onChange={handleInput} onSubmit={handleSubmit}>
      ...
      <Input placeholder={!formState.form.name ? 'Имя' : ''} type='text' name='name' value={formState.form.name}/>
      <Input placeholder={!formState.form.email ? 'E-mail' : ''} type='email' name='email' value={formState.form.email} />
      <PasswordInput name='password' placeholder={!formState.form.password ? 'Пароль' : ''} value={formState.form.password} />
      <Button type='primary' size="large" disabled={formState.sending} htmlType="submit">Зарегистрироваться</Button>
